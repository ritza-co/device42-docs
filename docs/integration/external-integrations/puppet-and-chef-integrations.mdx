...
This page is for administrators who use Puppet or Chef and want Device42 to share node data with them. By the end, either Device42 populates its asset inventory from Puppet or Chef node data, or Device42 acts as a Puppet External Node Classifier (ENC) that serves Puppet classes to your nodes.
...
- A running Device42 instance, with its IP address and an API username and password for the scripts to authenticate with
- Administrator access to your Puppet master or Chef server, including root access to edit configuration files
- Network connectivity between the Puppet master or Chef server and Device42
...
- Python v2.7.10 or later, with the packages in the script's `requirements.txt` installed

The Chef integration connector works with all Chef-supported versions of Chef software.
...
Getting the Device42 - Puppet ENC Integration up and running is a four step process.
