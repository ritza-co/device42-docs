...
This page is for automation engineers who want Rundeck to use the Device42 CMDB as its source of node data instead of a static resource file. By the end, Rundeck pulls its node list from Device42 and shows Device42 details such as location, operating system, and device type for each node.
...
- Network connectivity from Rundeck to Device42
- **Gradle**, only if you build the plugin from source instead of using the precompiled jar file
