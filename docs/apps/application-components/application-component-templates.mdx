...
Device4 automatically creates components for a [set list of applications](/apps/enterprise-application-dependency-mapping/adm-supported-applications.mdx). Other applications, such as in-house applications, won't have components automatically created for them, in which case you'll create a template. 

Device42 matches services during discovery to the template criteria you define and creates an Application Component for it. 

Device42 adds the Application Components to the [Application Components](/apps/application-components) list page under **Applications > Application Components**. Device records show the detected application under the **Application** tab.

## Example Application Components Templates

Java is a very common service you might find throughout your environment, but not all instances of it will be relevant to your application. You can tune which of the Java service instances you create Application
Components from. 

You could define the listening port for the Java service, the Command Argument, or other services that must be present on the device for it to create an Application Component.

In the example below, a Java [Service Instance](/apps/services/service-instances.mdx) listening on port `8181` that's found during discovery will be named “Jupiter”. The "Jupiter" application will be linked under the **Application** tab of the device record.
...
In the example below, more complex application instance criteria are used:

- The **Associate Service** is `OracleWebAssitant0`:
...
A `OracleWebAssitant0` Service instance found during discovery will be named “WebLogic” and found under the **Application** tab of the device record.
...
Application Component Templates allow you a lot granular control to avoid false positives. Though, all you need to enter on the Template form to start creating Application Components discovery are:
- a **Name**
